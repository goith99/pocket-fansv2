//! SwapStakeAndSave execution — TeamWin self-claim, DIRECT-TO-OWNER variant.
//!
//! Byte-for-byte identical to `execute_rule_staked` (delegated USDC pull, Orca
//! swap into an ephemeral wSOL account, unwrap to native SOL, Marinade deposit
//! signed by the `vault_sol` PDA) EXCEPT for ONE thing: the Marinade deposit's
//! `mint_to` account is the OWNER's mSOL ATA instead of the vault's. Marinade
//! does not check `mint_to`'s owner against the depositing `transfer_from`
//! authority (verified live on devnet), so the vault PDA still signs the deposit
//! while the minted mSOL lands straight in the owner's wallet.
//!
//! Why this exists: `execute_rule_staked` lands mSOL in the vault, requiring a
//! separate `withdraw_from_vault` to move it to the owner. This variant collapses
//! that to a SINGLE instruction with the full deposit output in the owner's
//! wallet. `execute_rule_staked` is left completely untouched; this is purely
//! additive — the exact same relationship `execute_rule_direct` has to
//! `execute_rule`.
//!
//! Flow (identical to execute_rule_staked except step 5's destination):
//!   1. guards: rule active, under cap, trigger_type == TeamWin, and
//!      Clock::unix_timestamp >= match_end_ts
//!   2. pull `amount_usdc` from the owner's USDC ATA into the vault's USDC ATA
//!      via the vault PDA's SPL delegation
//!   3. Orca Whirlpool swap USDC -> wSOL into an EPHEMERAL `stake_wsol` account
//!   4. close `stake_wsol` -> unwrap to native SOL in the `vault_sol` system PDA
//!   5. Marinade `deposit(swapped_lamports)` from `vault_sol` -> mints mSOL into
//!      the **OWNER's** mSOL ATA
//!   6. refund the residual `vault_sol` balance (the stake_wsol rent) to the owner
//!   7. executions_done += 1
//!
//! All amounts are raw base units (devUSDC 6dp, SOL/wSOL/mSOL 9dp).
...
pub struct ExecuteRuleStakedDirect<'info> {
    ...
    /// mSOL mint — Marinade's output. Marinade mints into `owner_msol_ata`;
    ...
    /// OWNER's mSOL account — Marinade deposit `mint_to` destination. THIS is the
    /// only account constraint that differs from execute_rule_staked, where this
    /// slot is the vault's mSOL ATA (token::authority = vault). Must already
    /// exist; the client creates it idempotently in the claim transaction.
    #[account(mut, token::mint = msol_mint, token::authority = owner)]
...
pub fn handler(ctx: Context<ExecuteRuleStakedDirect>, rule_id: u16) -> Result<()> {
    ...
    // 5) Marinade deposit(staked_lamports) from vault_sol -> mSOL straight to
    //    the OWNER's ATA (no vault hop, no withdraw).
    ...
    msg!(
        "Rule {} self-claimed (SwapStakeAndSave, direct): swapped {} raw devUSDC, staked {} lamports into Marinade -> mSOL straight to owner. executions_done = {}/{}",
        rule_id,
        amount_usdc,
        staked_lamports,
...
fn swap_usdc_to_wsol(
    _ctx: &Context<ExecuteRuleStakedDirect>,
    _amount_in: u64,
    _min_out: u64,
    _signer_seeds: &[&[&[u8]]],
) -> Result<()> {
