...
export default function ChallengeCard({
  ...
  // SELF-CLAIM MODEL: claimable once the fixture's match_end_ts has passed and
  // there's still capacity left. No oracle/admin decides this — it's a pure
  // client-side time check mirroring the on-chain guard in execute_rule.
  const claimable = r.isActive && r.executionsDone < r.maxExecutions && nowMs / 1000 >= r.matchEndTs;
    ...
    <div className="card p-3.5">
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-0.5 flex items-center gap-2 text-[13px] text-muted">
            <span className={`inline-flex items-center gap-1 font-semibold ${r.isActive ? "text-green" : "text-faint"}`}>
              <span className={`h-1.5 w-1.5 rounded-full ${r.isActive ? "bg-green" : "bg-faint"}`} />
      ...
      {claimable && (
        <div className="mt-3 flex items-start gap-2 rounded-xl bg-green-tint px-3 py-2.5 text-[13px] leading-relaxed text-green-deep">
          ...
          <p>Match day has passed for this challenge — tap <b>Claim</b> to move your savings now.</p>
        </div>
      )}

      {/* calm, informational note — team played and didn't win this match */}
      {r.isActive && !claimable && note && (
